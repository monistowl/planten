...
- `tools/plan9-qemu/replay-ns.sh` automates that flow: it starts QEMU with the shared namespace JSON, waits for the guest login (`sys/sys` by default), mounts the share, copies the JSON, runs `nsctl`, and issues `fsys main sync; fsys main halt`. The script depends on `expect`, which the CI workflow now installs along with QEMU and Python.
